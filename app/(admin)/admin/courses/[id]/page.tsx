...
export default function CourseDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (error || !course) return <div className="p-8 text-red-500 text-center">{error || "Course not found"}</div>;

  return (
    <div className="p-8 max-w-6xl mx-auto">
      <div className="mb-6 flex items-center gap-4">
        <Link href="/admin/courses" className="text-blue-600 hover:underline">← Back to Courses</Link>
        ...
        <div className="bg-white p-6 rounded-lg shadow border border-gray-100">
          ...
          <div className="space-y-2">
            {course.sections.map(s => (
              <div key={s.id} className="p-3 bg-gray-50 rounded border border-gray-100 text-gray-800 font-medium">
                {s.name}
              </div>
            ))}
            {course.sections.length === 0 && <p className="text-gray-400 text-sm italic">No sections added yet.</p>}
          </div>
        </div>

        {/* Labs Management */}
        <div className="bg-white p-6 rounded-lg shadow border border-gray-100">
          <h2 className="text-xl font-semibold mb-4 text-gray-700">Lab Groups</h2>
          ...
          <div className="space-y-2">
            {course.labs.map(l => (
              <div key={l.id} className="p-3 bg-gray-50 rounded border border-gray-100 text-gray-800 font-medium">
                {l.name}
              </div>
            ))}
            {course.labs.length === 0 && <p className="text-gray-400 text-sm italic">No lab groups added yet.</p>}
          </div>
        </div>

        {/* Coordinator Management */}
        <div className="bg-white p-6 rounded-lg shadow border border-gray-100 md:col-span-2">
          <h2 className="text-xl font-semibold mb-4 text-gray-700">Assigned Coordinators</h2>
          <form onSubmit={assignCoordinator} className="flex gap-2 mb-6 max-w-md">
            ...
            <button className="bg-blue-600 text-white px-6 py-2 rounded hover:bg-blue-700 font-bold">Assign</button>
          </form>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {course.coordinators.map(c => (
              <div key={c.id} className="p-4 border rounded-lg flex justify-between items-center bg-blue-50 border-blue-100">
                <div>
                  <p className="font-bold text-blue-900">{c.name}</p>
                  <p className="text-xs text-blue-700">{c.email}</p>
                </div>
                <button
            ...
            {course.coordinators.length === 0 && <p className="text-gray-400 italic">No coordinators assigned to this course yet.</p>}
          </div>
        </div>
      </div>
    </div>
